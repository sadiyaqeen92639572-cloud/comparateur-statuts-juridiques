...
// ACRE micro : 25% de réduction des cotisations sociales jusqu'à la fin du 3e trimestre civil
// suivant le début d'activité (pas de barème dégressif pour le micro, pas de variation
// 2025/2026). Source : service-public.gouv.fr F11677. Réduction paramétrique via
// constantes.acre_micro.reduction — jamais de 0.25 en dur ici.
function resolveAcreReduction(params, constantes) {
  if (!params || !params.acre) return 0;
  const acre = (constantes && constantes.acre_micro) || {};
  const r = acre.reduction;
  return (typeof r === "number" && r > 0 && r < 1) ? r : 0;
}

function calculerIndividuel(statut, params, constantes = {}) {
  const { ca, type_activite, charges_reelles = 0, option_vlf = false, tmi = 0.30 } = params;

  if (statut.regime_fiscal === "micro-fiscal") {
    const acreRed = resolveAcreReduction(params, constantes);
    const tauxCotis = statut.taux_cotisations_sociales[type_activite] * (1 - acreRed);
...
// EURL à l'IR (régime par défaut de l'associé unique personne physique) : pas d'IS, pas de
// couche dividendes/flat-tax. Tout le bénéfice est le revenu du gérant, soumis cotisations TNS
// puis IR au barème. Le forfait TNS (taux_cotisations_tns_estime) est une APPROXIMATION —
// l'assiette sociale réelle 2025 applique un abattement de 26% et réintègre la CSG/CRDS non
// déductible (~2,9%) dans le revenu imposable. Résultat présenté comme "estimation" côté widget.
function calculerEurlIR(statut, params) {
  const { ca, charges_exploitation = 0, tmi = 0.30 } = params;
  const revenu_activite = Math.max(0, ca - charges_exploitation);
  const cotisations_sociales = revenu_activite * (statut.taux_cotisations_tns_estime || 0.45);
  const revenu_imposable = revenu_activite - cotisations_sociales;
  const impot_revenu = appliquerTmi(revenu_imposable, tmi);
  const revenu_net_apres_impot = revenu_imposable - impot_revenu;
  return {
    revenu_imposable,
    cotisations_sociales,
    cotisations_sociales_gerant: cotisations_sociales,
    is_du: 0,
    impot_revenu,
    impot_revenu_remuneration: impot_revenu,
    revenu_net_apres_impot,
    taux_prelevement_global: ca > 0 ? (cotisations_sociales + impot_revenu) / ca : 0
  };
}
...
function calculerStatut(statutId, params, statutsData) {
  ...
  let resultat;
  if (statutId === "eurl" && params && params.regime_fiscal === "IR") {
    resultat = calculerEurlIR(statut, params);
  } else if (statut.categorie === "individuel") {
    resultat = calculerIndividuel(statut, params, statutsData.constantes_communes);
  } else if (statut.categorie === "portage_salarial") {
    resultat = calculerPortageSalarial(statut, params);
  } else {
    resultat = calculerSociete(statut, params, statutsData.constantes_communes);
  }
